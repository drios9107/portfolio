      ...
      <q-card flat class="row flex justify-between project-item">
        <div class="thumbnail" @click="()=>openModal(project)">
          <img :src="project?.images?.[0]??'/no-img.svg'"
            :class="`fit-cover ${project?.images?.[0]&&'cursor-pointer'} project-thumbnail`" style="border-radius: 12px;" />
        </div>
  ...
  <q-dialog v-model="dialog" persistent>
    <q-card style="width: 600px; max-width: 90vw;">
      ...
      <div class="project-thumbnail" v-if="activeProject?.images?.length === 1">
        <img :src="activeProject?.images?.[0]"
          class="full-width full-height fit-cover carousel-img" />
      </div>
...
.carousel-img{
  border:4px solid grey; border-top-left-radius: 12px; border-bottom-left-radius: 12px;
}
</style>